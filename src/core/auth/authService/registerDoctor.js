const doctorData = require("../../user/doctor/doctorModel");
...
const registerDoctor = async (name, secondName, phone, email, speciality, password) => {
    const client = await doctorData.findOne({email});
    if (client) {
      throw createError(409, `Email is already used`);
    }
   const hashPassword = await bcrypt.hash(password, 10);
   const activationLink = uuid.v4();
   const newClient = await doctorData.create({name, secondName, phone, email, speciality, password: hashPassword, activationLink});
